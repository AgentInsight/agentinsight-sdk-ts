import { getGlobalLogger } from "@agentinsight-sdk/core";
import {
...
export class CallbackHandler extends BaseCallbackHandler {
  name = "AgentInsightCallbackHandler";
  ...
  constructor(params?: CallbackHandlerOptions) {
    super();
    ...
    this.promptToParentRunMap = new Map<string, AgentInsightPrompt>();
  }
  ...
  async handleChainStart(
    ...
    try {
      ...
      this.registerAgentInsightPrompt(parentRunId, metadata);
      ...
      if (!parentRunId) {
        ...
        const traceUserId =
          metadata &&
          "agentInsightUserId" in metadata &&
          typeof metadata["agentInsightUserId"] === "string"
            ? metadata["agentInsightUserId"]
            : this.userId;

        const traceSessionId =
          metadata &&
          "agentInsightSessionId" in metadata &&
          typeof metadata["agentInsightSessionId"] === "string"
            ? metadata["agentInsightSessionId"]
            : this.sessionId;
  ...
  async handleGenerationStart(
    ...
    if (registeredPrompt && parentRunId) {
      this.deregisterAgentInsightPrompt(parentRunId);
    }
  ...
  async handleChainEnd(
    ...
    try {
      ...
      this.deregisterAgentInsightPrompt(runId);
    } catch (e) {
      this.logger.debug(e instanceof Error ? e.message : String(e));
    }
  }
  ...
  private registerAgentInsightPrompt(
    parentRunId?: string,
    ...
    */
    if (metadata && "agentInsightPrompt" in metadata && parentRunId) {
      this.promptToParentRunMap.set(
        parentRunId,
        metadata.agentInsightPrompt as AgentInsightPrompt,
      );
    }
  }

  private deregisterAgentInsightPrompt(runId: string): void {
  ...
  private startAndRegisterOtelSpan(params: {
    ...
    const observationAttributes = {
      ...
      ...attributes,
    } as AgentInsightObservationAttributes;
    ...
    const observation =
      type === "generation"
        ? startActiveObservation(
            ...
            (gen) => {
              gen.update(
                observationAttributes as AgentInsightGenerationAttributes,
              );
              return gen;
            },
            {
        ...
        : type === "tool"
          ? startActiveObservation(
              ...
              (tool) => {
                tool.update(
                  observationAttributes as AgentInsightSpanAttributes,
                );
                return tool;
              },
              {
  ...
  private handleOtelSpanEnd(params: {
    runId: string;
    attributes?: AgentInsightGenerationAttributes | AgentInsightSpanAttributes;
  ...
  private joinTagsAndMetaData(
    ...
    return this.stripAgentInsightKeysFromMetadata(finalDict);
  }

  private stripAgentInsightKeysFromMetadata(
    metadata?: Record<string, unknown>,
  ): Record<string, unknown> | undefined {
    if (!metadata) {
      return;
    ...
    const agentInsightKeys = [
      "agentInsightPrompt",
      "agentInsightUserId",
      "agentInsightSessionId",
    ];

    return Object.fromEntries(
      Object.entries(metadata).filter(
        ([key, _]) => !agentInsightKeys.includes(key),
      ),
    );
  }
