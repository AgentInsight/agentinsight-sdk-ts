...
/**
 * Wraps an OpenAI SDK client with automatic AgentInsight tracing.
 *
 ...
 * automatically captured as AgentInsight generations.
 *
 * @param sdk - The OpenAI SDK client instance to wrap with tracing
 * @param agentInsightConfig - Optional configuration for tracing behavior
 ...
 * @example
 * ```typescript
 * // Using with AgentInsight prompt management
 * const openai = observeOpenAI(new OpenAI({
 *   apiKey: process.env.OPENAI_API_KEY
 * }), {
 *   agentInsightPrompt: {
...
export const observeOpenAI = <SDKType extends object>(
  sdk: SDKType,
  agentInsightConfig?: AgentInsightConfig,
): SDKType => {
  return new Proxy(sdk, {
    get(wrappedSdk, propKey, proxy) {
      ...
      const generationName =
        agentInsightConfig?.generationName ?? defaultGenerationName;
      const config = { ...agentInsightConfig, generationName };
