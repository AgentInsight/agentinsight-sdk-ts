...
import type { AgentInsightConfig } from "./types.js";
...
/**
 * Wraps a method with AgentInsight tracing functionality.
 *
 * This function creates a wrapper around OpenAI SDK methods that automatically
 * creates AgentInsight generations, captures input/output data, handles streaming
 ...
 * @returns A wrapped version of the method that creates AgentInsight traces
 *
 * @internal
 */
export const withTracing = <T extends GenericMethod>(
  tracedMethod: T,
  config?: AgentInsightConfig & Required<{ generationName: string }>,
 ...
 *
 * This function creates an AgentInsight generation, executes the original method,
 ...
 * @param config - AgentInsight configuration options
    ...
    () => {
      ...
      const generation = startObservation(
        config?.generationName ?? "OpenAI-completion",
        {
          ...
          prompt: config?.agentInsightPrompt,
...
/**
 * Wraps an async iterable (streaming response) with AgentInsight tracing.
 *
 * This function handles streaming OpenAI responses by collecting chunks,
 * parsing usage information, and updating the AgentInsight generation with
 ...
 * @param generation - The AgentInsight generation to update with stream data
