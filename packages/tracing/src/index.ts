...
export {
  setAgentInsightTracerProvider,
  getAgentInsightTracerProvider,
  getAgentInsightTracer,
...
export { AgentInsightOtelSpanAttributes } from "@agentinsight-sdk/core";
...
export type StartObservationOpts = StartObservationOptions & {
  ...
  asType?: AgentInsightObservationType;
};
...
/**
 * Creates an OpenTelemetry span with the AgentInsight tracer.
 *
...
function createOtelSpan(params: {
  ...
  return getAgentInsightTracer().startSpan(
    params.name,
...
/**
 * Creates and starts a new AgentInsight observation with automatic TypeScript type inference.
 *
 * This is the primary method for creating observations in AgentInsight. It supports multiple
...
export function startObservation(
  ...
  switch (asType) {
    case "generation":
      return new AgentInsightGeneration({
        otelSpan,
        attributes,
      });

    case "embedding":
      return new AgentInsightEmbedding({
        otelSpan,
        attributes,
      });

    case "agent":
      return new AgentInsightAgent({
        otelSpan,
        attributes,
      });

    case "tool":
      return new AgentInsightTool({
        otelSpan,
        attributes,
      });

    case "chain":
      return new AgentInsightChain({
        otelSpan,
        attributes,
      });

    case "retriever":
      return new AgentInsightRetriever({
        otelSpan,
        attributes,
      });

    case "evaluator":
      return new AgentInsightEvaluator({
        otelSpan,
        attributes,
      });

    case "guardrail":
      return new AgentInsightGuardrail({
        otelSpan,
        attributes,
      });
    ...
    case "event": {
      ...
      return new AgentInsightEvent({
        otelSpan,
        attributes: attributes as AgentInsightEventAttributes,
        timestamp,
      });
    }
    case "span":
    default:
      return new AgentInsightSpan({
        otelSpan,
        attributes: attributes as AgentInsightSpanAttributes,
      });
  }
}
...
export function startActiveObservation<
  F extends (generation: AgentInsightGeneration) => unknown,
>(
  name: string,
  fn: F,
...
export function startActiveObservation<
  F extends (embedding: AgentInsightEmbedding) => unknown,
>(
  name: string,
  fn: F,
...
export function startActiveObservation<
  F extends (agent: AgentInsightAgent) => unknown,
>(
  name: string,
  fn: F,
...
export function startActiveObservation<
  F extends (tool: AgentInsightTool) => unknown,
>(
  name: string,
  fn: F,
...
export function startActiveObservation<
  F extends (chain: AgentInsightChain) => unknown,
>(
  name: string,
  fn: F,
...
export function startActiveObservation<
  F extends (retriever: AgentInsightRetriever) => unknown,
>(
  name: string,
  fn: F,
...
export function startActiveObservation<
  F extends (evaluator: AgentInsightEvaluator) => unknown,
>(
  name: string,
  fn: F,
...
export function startActiveObservation<
  F extends (guardrail: AgentInsightGuardrail) => unknown,
>(
  name: string,
  fn: F,
...
export function startActiveObservation<
  F extends (span: AgentInsightSpan) => unknown,
>(
  name: string,
  fn: F,
 ...
 * @example
 * ```typescript
 * import { startActiveObservation } from '@agentinsight-sdk/tracing';
 *
 * // Span for general operations (default)
 ...
 */
export function startActiveObservation<
  F extends (observation: AgentInsightSpan | AgentInsightGeneration) => unknown,
>(name: string, fn: F, options?: StartActiveObservationOpts): ReturnType<F> {
  const { asType = "span", ...observationOptions } = options || {};

  return getAgentInsightTracer().startActiveSpan(
    name,
    ...
    (span) => {
      try {
        let observation: AgentInsightObservation;

        switch (asType) {
          case "generation":
            observation = new AgentInsightGeneration({
              otelSpan: span,
            });
            break;

          case "embedding":
            observation = new AgentInsightEmbedding({
              otelSpan: span,
            });
            break;

          case "agent":
            observation = new AgentInsightAgent({
              otelSpan: span,
            });
            break;

          case "tool":
            observation = new AgentInsightTool({
              otelSpan: span,
            });
            break;

          case "chain":
            observation = new AgentInsightChain({
              otelSpan: span,
            });
            break;

          case "retriever":
            observation = new AgentInsightRetriever({
              otelSpan: span,
            });
            break;

          case "evaluator":
            observation = new AgentInsightEvaluator({
              otelSpan: span,
            });
            break;

          case "guardrail":
            observation = new AgentInsightGuardrail({
              otelSpan: span,
            });
            break;
          ...
          case "event": {
            const timestamp = observationOptions?.startTime ?? new Date();
            observation = new AgentInsightEvent({
              otelSpan: span,
              timestamp,
            });
            break;
          ...
          case "span":
          default:
            observation = new AgentInsightSpan({
              otelSpan: span,
            });
        }
 ...
 *
 * @deprecated This is a legacy function for backward compatibility with AgentInsight platform
 ...
 * @example
 * ```typescript
 * import { setActiveTraceIO } from '@agentinsight-sdk/tracing';
 *
 ...
 * @public
 */
export function setActiveTraceIO(attributes: AgentInsightTraceAttributes) {
 ...
 * @example
 * ```typescript
 * import { setActiveTraceAsPublic, startActiveObservation } from '@agentinsight-sdk/tracing';
 *
 ...
 * and updates it with AgentInsight-specific attributes. It supports all observation types
 ...
 * @example
 * ```typescript
 * import { updateActiveObservation, startActiveObservation } from '@agentinsight-sdk/tracing';
 *
...
export function updateActiveObservation(
  ...
  if (!options?.asType) {
    otelAttributes[AgentInsightOtelSpanAttributes.OBSERVATION_TYPE] = undefined;
  }
...
/**
 * Decorator function that automatically wraps any function with AgentInsight observability.
 *
...
export function observe<T extends (...args: any[]) => any>(
  ...
  const wrappedFunction = function (
    ...
    try {
      ...
      const result = context.with(activeContext, () => boundFn(...args));
 ...
 *               non-W3C compliant IDs with AgentInsight trace IDs. This allows you to later
 *               have a method available for scoring the AgentInsight trace given only the
 ...
 * });
 *
 * // Correlating external IDs with AgentInsight traces
