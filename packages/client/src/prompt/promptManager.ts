...
import { AgentInsightPromptCache } from "./promptCache.js";
import {
  ChatPromptClient,
  TextPromptClient,
  AgentInsightPromptClient,
...
/**
 * Manager for prompt operations in AgentInsight.
 *
...
export class PromptManager {
  private cache: AgentInsightPromptCache;
   ...
   * @internal
   */
  constructor(params: { apiClient: AgentInsightAPIClient }) {
    const { apiClient } = params;

    this.apiClient = apiClient;
    this.cache = new AgentInsightPromptCache();
  }
  ...
  /**
   * Creates a new prompt in AgentInsight.
   *
   ...
   * const textPrompt = await agentInsight.prompt.create({
   *   name: "greeting",
   *   prompt: "Hello {{name}}!",
   *   type: "text"
   * });
   ...
   * const chatPrompt = await agentInsight.prompt.create({
   *   name: "conversation",
   *   type: "chat",
   *   prompt: [
  ...
  async create(
    body:
      ...
      | CreateChatPromptBodyWithPlaceholders,
  ): Promise<AgentInsightPromptClient> {
   ...
   * @example
   * ```typescript
   * const updatedPrompt = await agentInsight.prompt.update({
   *   name: "my-prompt",
   *   version: 1,
   ...
   *
   * The AgentInsight SDK prompt cache is invalidated for all cached versions with the specified name.
   *
   ...
   *
   * @throws {AgentInsightAPI.NotFoundError} If the prompt does not exist
   * @throws {AgentInsightAPI.Error} If the API request fails
   *
   * @example
   * ```typescript
   * // Delete all versions of a prompt
   * await agentInsight.prompt.delete("my-prompt");
   *
   * // Delete specific version
   * await agentInsight.prompt.delete("my-prompt", { version: 2 });
   *
   * // Delete all versions with a specific label
   * await agentInsight.prompt.delete("my-prompt", { label: "staging" });
   * ```
   */
  async delete(
    name: string,
   ...
   * const v2Prompt = await agentInsight.prompt.get("my-prompt", {
   *   version: 2
   * });
   *
   * // Get with label filter
   * const prodPrompt = await agentInsight.prompt.get("my-prompt", {
   *   label: "production"
   * });
   *
   * // Get with fallback
   * const promptWithFallback = await agentInsight.prompt.get("my-prompt", {
   *   type: "text",
   *   fallback: "Hello {{name}}!"
   * });
   * ```
  ...
  private async fetchPromptAndUpdateCache(params: {
    ...
    try {
      ...
      let prompt: AgentInsightPromptClient;
      if (data.type === "chat") {
