...
/**
 * Manager for creating and batching score events in AgentInsight.
 *
...
export class ScoreManager {
  ...
  constructor(params: { apiClient: AgentInsightAPIClient }) {
    ...
    const envFlushAtCount = getEnv("AGENTINSIGHT_FLUSH_AT");
    const envFlushIntervalSeconds = getEnv("AGENTINSIGHT_FLUSH_INTERVAL");
  ...
  public create(data: ScoreBody): void {
    const scoreData: ScoreBody = {
      ...
      environment:
        data.environment ?? getEnv("AGENTINSIGHT_TRACING_ENVIRONMENT"),
    };
   ...
   * @example
   * ```typescript
   * import { startSpan } from '@agentinsight-sdk/tracing';
   *
   * const span = startSpan({ name: "my-operation" });
   * agentInsight.score.observation(
   *   { otelSpan: span },
   ...
   * @example
   * ```typescript
   * import { startActiveSpan } from '@agentinsight-sdk/tracing';
   *
   * startActiveSpan({ name: "my-operation" }, (span) => {
   *   // Inside the active span
   *   agentInsight.score.activeObservation({
   *     name: "relevance",
   *     value: 0.95
   *   });
   * });
   ...
   * startActiveSpan({ name: "my-operation" }, (span) => {
   *   // Inside the active span
   *   agentInsight.score.activeTrace({
  ...
  /**
   * Flushes all pending score events to the AgentInsight API.
   *
   ...
   * await agentInsight.score.flush(); // Ensures the score is sent immediately
   * ```
   */
  public async flush() {
   ...
   * all score data is sent to AgentInsight.
   *
   ...
   * await agentInsight.score.shutdown();
   * ```
   */
