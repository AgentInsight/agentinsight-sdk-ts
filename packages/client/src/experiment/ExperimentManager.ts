 ...
 * ```
 *
 * @example Using with AgentInsight datasets
 * ```typescript
...
export class ExperimentManager {
   ...
   * @param params.agentInsightClient - The AgentInsight client instance for API communication
   * @internal
   */
   ...
   * 4. Links results to dataset runs (for AgentInsight datasets)
   * 5. Stores all scores and traces in AgentInsight
   *
   ...
   *   - datasetRunId: ID of the dataset run (if using AgentInsight datasets)
  ...
  >(
    ...
    if (!this.isOtelRegistered()) {
      this.logger.warn(
        "OpenTelemetry has not been set up. Traces will not be sent to AgentInsight.See our docs on how to set up OpenTelemetry: https://agentinsight.com/docs/observability/sdk/typescript/setup#tracing-setup",
      );
    }
    ...
    if (datasetRunId && data.length > 0 && "datasetId" in data[0]) {
      ...
      const projectUrl = (
        await this.agentInsightClient.getTraceUrl("mock")
      ).split("/traces")[0];
    ...
    if (runEvaluators && runEvaluators?.length > 0) {
      ...
      if (datasetRunId) {
        runEvaluations.forEach((runEval) =>
          this.agentInsightClient.score.create({ datasetRunId, ...runEval }),
        );
      }
    }

    await this.agentInsightClient.score.flush();
   ...
   * 4. Stores evaluation scores in AgentInsight
  ...
  >(params: {
    ...
    const { output, traceId, observationId, datasetRunId } =
      await startActiveObservation("experiment-item-run", async (span) => {
        ...
        if (datasetItemId) {
          try {
            const result =
              await this.agentInsightClient.api.datasetRunItems.create({
    ...
    for (const ev of evals) {
      this.agentInsightClient.score.create({
        traceId,
        observationId,
        ...ev,
      });
   ...
   * - Links to dataset runs in the AgentInsight UI
   *
   * @param params - Formatting parameters
   * @param params.datasetRunUrl - Optional URL to the dataset run in AgentInsight UI
   ...
   *
   *    Dataset Item:
   *    https://agent.glodebridge.com/project/123/datasets/456/items/789
   *
   *    Trace:
   *    https://agent.glodebridge.com/project/123/traces/abc123
   *
   * ──────────────────────────────────────────────────
   ...
   *
   * 🔗 Dataset Run:
   *    https://agent.glodebridge.com/project/123/datasets/456/runs/def456
   * ```
   *
   * @internal
   */
  ...
  >(params: {
    ...
    if (includeItemResults) {
      for (let index = 0; index < itemResults.length; index++) {
        ...
        if (result.traceId) {
          const traceUrl = await this.agentInsightClient.getTraceUrl(
            result.traceId,
          );
          output += `\n   Trace:\n   ${traceUrl}\n`;
        }
