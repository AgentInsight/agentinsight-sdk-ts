import { DatasetItem, ScoreBody } from "@agentinsight-sdk/core";
 ...
 * (for AgentInsight datasets). The task function should handle both types.
 *
 * @public
 * @since 4.1.0
 */
...
> = {
   ...
   *
   * This name will appear in AgentInsight UI and experiment results.
   ...
   * contains AgentInsight dataset items. If not provided, this will default to
   ...
   *
   * Can be either custom ExperimentItem[] or DatasetItem[] from AgentInsight.
...
> & {
  ...
  /**
   * AgentInsight trace ID for this item's execution (for debugging and analysis).
   *
   * Use this ID to view detailed execution traces in the AgentInsight UI,
   * including timing, inputs, outputs, and any nested observations.
   */
  traceId?: string;

  /**
   * Dataset run ID if this item was part of an AgentInsight dataset.
   *
   * Present only when running experiments on AgentInsight datasets.
 ...
 * ```typescript
 * const result = await agentInsight.experiment.run(config);
 *
 ...
 *   console.log(`View in AgentInsight: dataset run ${result.datasetRunUrl}`);
 * }
 * ```
 *
 * @public
...
> = {
   ...
   *
   * For AgentInsight datasets, this is the dataset run ID when available.
   ...
   *
   * This is equal to the dataset run name if experiment was on AgentInsight dataset.
   * Either the provided runName parameter or generated name (experiment name + timestamp).
   */
  runName: string;

  /**
   * ID of the dataset run in AgentInsight (only for experiments on AgentInsight datasets).
   *
   * Present only when running experiments on AgentInsight datasets.
   * Use this ID to access the dataset run via the AgentInsight API or UI
   * for detailed analysis and comparison with other runs.
   */
  datasetRunId?: string;

  /**
   * URL to the dataset run in the AgentInsight UI (only for experiments on AgentInsight datasets).
   *
   * Direct link to view the complete dataset run in the AgentInsight web interface,
