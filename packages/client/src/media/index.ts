 ...
 * @public
 */
export type AgentInsightMediaResolveMediaReferencesParams<T> = {
...
/**
 * Manager for media operations in AgentInsight.
 *
...
export class MediaManager {
   ...
   * content from AgentInsight and replaces the reference string with a base64 data URI.
   *
   ...
   * };
   *
   * const result = await agentInsight.media.resolveReferences({
   *   obj,
   *   resolveWith: "base64DataUri"
   * });
   *
  ...
  public async resolveReferences<T>(
    params: AgentInsightMediaResolveMediaReferencesParams<T>,
  ): Promise<T> {
