...
export const AgentInsightOtelContextKeys: Record<PropagatedKey, symbol> = {
...
const AGENTINSIGHT_BAGGAGE_PREFIX = "langfuse_";
const AGENTINSIGHT_BAGGAGE_TAGS_SEPARATOR = ",";
...
export interface PropagateAttributesParams {
  ...
  /**
   * **INTERNAL USE ONLY** - For AgentInsight experiment framework.
   *
   * This parameter is used internally by the AgentInsight experiment system to propagate
 ...
 *
 * **Why this matters**: AgentInsight aggregation queries (e.g., total cost by userId,
 ...
 *
 * ```typescript
 * import { startActiveObservation, propagateAttributes } from '@agentinsight-sdk/tracing';
 *
...
export function getPropagatedAttributesFromContext(
  ...
  if (baggage) {
    baggage.getAllEntries().forEach(([baggageKey, baggageEntry]) => {
      if (baggageKey.startsWith(AGENTINSIGHT_BAGGAGE_PREFIX)) {
        ...
        if (spanKey) {
          ...
          propagatedAttributes[spanKey] = isMergedTags
            ? baggageEntry.value.split(AGENTINSIGHT_BAGGAGE_TAGS_SEPARATOR)
            : baggageEntry.value;
        }
      }
    });
  ...
  const userId = context.getValue(AgentInsightOtelContextKeys["userId"]);
  ...
  const sessionId = context.getValue(AgentInsightOtelContextKeys["sessionId"]);
  ...
  const version = context.getValue(AgentInsightOtelContextKeys["version"]);
  ...
  const traceName = context.getValue(AgentInsightOtelContextKeys["traceName"]);
  ...
  const tags = context.getValue(AgentInsightOtelContextKeys["tags"]);
  ...
  const metadata = context.getValue(AgentInsightOtelContextKeys["metadata"]);
  if (metadata && typeof metadata === "object" && metadata !== null) {
    for (const [k, v] of Object.entries(metadata)) {
      const spanKey = `${AgentInsightOtelSpanAttributes.TRACE_METADATA}.${k}`;
  ...
  for (const key of experimentKeys) {
    const contextKey = AgentInsightOtelContextKeys[key];
  ...
  if (
    ...
    propagatedAttributes[AgentInsightOtelSpanAttributes.ENVIRONMENT] =
      AGENTINSIGHT_SDK_EXPERIMENT_ENVIRONMENT;
  }
  ...
  if (span && span.isRecording()) {
    if (key === "metadata") {
      for (const [k, v] of Object.entries(mergedMetadata)) {
        span.setAttribute(
          `${AgentInsightOtelSpanAttributes.TRACE_METADATA}.${k}`,
          v,
        );
      }
    } else if (key === "tags") {
  ...
  if (asBaggage) {
    ...
    if (key === "metadata") {
      ...
      baggage = baggage.setEntry(baggageKey, {
        value: mergedTags.join(AGENTINSIGHT_BAGGAGE_TAGS_SEPARATOR),
      });
    } else {
...
function getContextMergedTags(context: Context, newTags: string[]): string[] {
  const existingTags = context.getValue(AgentInsightOtelContextKeys["tags"]);
...
function getContextKeyForPropagatedKey(key: PropagatedKey): symbol {
  return AgentInsightOtelContextKeys[key];
}

function getSpanKeyForPropagatedKey(key: PropagatedKey): string {
  switch (key) {
    case "userId":
      return AgentInsightOtelSpanAttributes.TRACE_USER_ID;
    case "sessionId":
      return AgentInsightOtelSpanAttributes.TRACE_SESSION_ID;
    case "version":
      return AgentInsightOtelSpanAttributes.VERSION;
    case "traceName":
      return AgentInsightOtelSpanAttributes.TRACE_NAME;
    case "metadata":
      return AgentInsightOtelSpanAttributes.TRACE_METADATA;
    case "tags":
      return AgentInsightOtelSpanAttributes.TRACE_TAGS;
    case "experimentId":
      return AgentInsightOtelSpanAttributes.EXPERIMENT_ID;
    case "experimentName":
      return AgentInsightOtelSpanAttributes.EXPERIMENT_NAME;
    case "experimentMetadata":
      return AgentInsightOtelSpanAttributes.EXPERIMENT_METADATA;
    case "experimentDatasetId":
      return AgentInsightOtelSpanAttributes.EXPERIMENT_DATASET_ID;
    case "experimentItemId":
      return AgentInsightOtelSpanAttributes.EXPERIMENT_ITEM_ID;
    case "experimentItemMetadata":
      return AgentInsightOtelSpanAttributes.EXPERIMENT_ITEM_METADATA;
    case "experimentItemRootObservationId":
      return AgentInsightOtelSpanAttributes.EXPERIMENT_ITEM_ROOT_OBSERVATION_ID;
    default: {
...
function getBaggageKeyForPropagatedKey(key: PropagatedKey): string {
  ...
  switch (key) {
    case "userId":
      return `${AGENTINSIGHT_BAGGAGE_PREFIX}user_id`;
    case "sessionId":
      return `${AGENTINSIGHT_BAGGAGE_PREFIX}session_id`;
    case "version":
      return `${AGENTINSIGHT_BAGGAGE_PREFIX}version`;
    case "traceName":
      return `${AGENTINSIGHT_BAGGAGE_PREFIX}trace_name`;
    case "metadata":
      return `${AGENTINSIGHT_BAGGAGE_PREFIX}metadata`;
    case "tags":
      return `${AGENTINSIGHT_BAGGAGE_PREFIX}tags`;
    case "experimentId":
      return `${AGENTINSIGHT_BAGGAGE_PREFIX}experiment_id`;
    case "experimentName":
      return `${AGENTINSIGHT_BAGGAGE_PREFIX}experiment_name`;
    case "experimentMetadata":
      return `${AGENTINSIGHT_BAGGAGE_PREFIX}experiment_metadata`;
    case "experimentDatasetId":
      return `${AGENTINSIGHT_BAGGAGE_PREFIX}experiment_dataset_id`;
    case "experimentItemId":
      return `${AGENTINSIGHT_BAGGAGE_PREFIX}experiment_item_id`;
    case "experimentItemMetadata":
      return `${AGENTINSIGHT_BAGGAGE_PREFIX}experiment_item_metadata`;
    case "experimentItemRootObservationId":
      return `${AGENTINSIGHT_BAGGAGE_PREFIX}experiment_item_root_observation_id`;
    default: {
      const fallback: never = key;
...
function getSpanKeyFromBaggageKey(baggageKey: string): string | undefined {
  if (!baggageKey.startsWith(AGENTINSIGHT_BAGGAGE_PREFIX)) return;

  const suffix = baggageKey.slice(AGENTINSIGHT_BAGGAGE_PREFIX.length);
  ...
  if (suffix.startsWith("metadata_")) {
    ...
    return `${AgentInsightOtelSpanAttributes.TRACE_METADATA}.${metadataKey}`;
  }
