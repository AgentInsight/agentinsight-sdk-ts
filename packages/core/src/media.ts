...
/**
 * Parameters for creating a AgentInsightMedia instance.
 *
 ...
 * @public
 */
export type AgentInsightMediaParams =
  | {
 ...
 * const media1 = new AgentInsightMedia({
 *   source: "base64_data_uri",
 ...
 *
 * // From raw bytes
 * const media2 = new AgentInsightMedia({
 *   source: "bytes",
 ...
 * @public
 */
export class AgentInsightMedia {
  _contentBytes?: Uint8Array;
  _contentType?: MediaContentType;
  _source?: string;

  /**
   * Creates a new AgentInsightMedia instance.
   *
   ...
   * const media = new AgentInsightMedia({
   *   source: "base64_data_uri",
   *   base64DataUri: "data:image/jpeg;base64,/9j/4AAQSkZJRgABAQAAAQ..."
   * });
   * ```
   */
  constructor(params: AgentInsightMediaParams) {
    const { source } = params;
  ...
  async getSha256Hash(): Promise<string | undefined> {
    ...
    try {
      const hash = await crypto.subtle.digest(
        "SHA-256",
        this._contentBytes as unknown as ArrayBuffer,
      );

      return bytesToBase64(new Uint8Array(hash));
    } catch (error) {
      getGlobalLogger().warn(
        "[AgentInsight] Failed to generate SHA-256 hash for media content:",
        error,
      );
   ...
   * content when the trace is viewed in AgentInsight.
   *
