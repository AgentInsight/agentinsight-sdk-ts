...
function parseLogLevelFromEnv(): LogLevel | undefined {
  if (typeof process === "object" && "env" in process) {
    if (getEnv("AGENTINSIGHT_DEBUG")?.toLowerCase() === "true")
      return LogLevel.DEBUG;

    const envValue = getEnv("AGENTINSIGHT_LOG_LEVEL");
 ...
 * @example
 * ```typescript
 * const logger = new Logger({ level: LogLevel.DEBUG, prefix: 'AgentInsight SDK' });
...
export class Logger {
  ...
  private formatMessage(level: string, message: string): string {
    const timestamp = this.config.enableTimestamp
      ...
      : "";
    const prefix = this.config.prefix || "[AgentInsight SDK]";
 ...
 * @example
 * ```typescript
 * import { logger } from '@agentinsight-sdk/core';
 *
