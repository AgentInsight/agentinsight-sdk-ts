...
describe("Experiment Attribute Propagation", () => {
  ...
  let langfuse: AgentInsightClient;

  beforeEach(async () => {
    testEnv = await setupTestEnvironment();
    langfuse = new AgentInsightClient({
      publicKey: "test-pk",
      secretKey: "test-sk",
  ...
  describe("Basic Experiment Propagation", () => {
    it("should propagate experiment attributes to child spans", async () => {
      ...
      expect(
        rootSpan?.attributes[AgentInsightOtelSpanAttributes.EXPERIMENT_ID],
      ).toBeDefined();
      expect(
        rootSpan?.attributes[AgentInsightOtelSpanAttributes.EXPERIMENT_NAME],
      ).toBeDefined();
      expect(
        rootSpan?.attributes[AgentInsightOtelSpanAttributes.EXPERIMENT_ITEM_ID],
      ).toBeDefined();

      // Child span should inherit experiment attributes
      expect(
        childSpan?.attributes[AgentInsightOtelSpanAttributes.EXPERIMENT_ID],
      ).toBe(
        rootSpan?.attributes[AgentInsightOtelSpanAttributes.EXPERIMENT_ID],
      );
      expect(
        childSpan?.attributes[AgentInsightOtelSpanAttributes.EXPERIMENT_NAME],
      ).toBe(
        rootSpan?.attributes[AgentInsightOtelSpanAttributes.EXPERIMENT_NAME],
      );
      expect(
    ...
    it("should propagate experiment item metadata to child spans", async () => {
      ...
      expect(
        childSpan?.attributes[
          AgentInsightOtelSpanAttributes.EXPERIMENT_ITEM_METADATA
        ],
      ).toBeDefined();
      const metadata = JSON.parse(
        childSpan?.attributes[
  ...
  describe("Nested Spans", () => {
    it("should propagate to multiple levels of nested child spans", async () => {
      ...
      expect(
        level1?.attributes[AgentInsightOtelSpanAttributes.EXPERIMENT_ID],
      ).toBe(experimentId);
      expect(
        level2?.attributes[AgentInsightOtelSpanAttributes.EXPERIMENT_ID],
      ).toBe(experimentId);
      expect(
        level3?.attributes[AgentInsightOtelSpanAttributes.EXPERIMENT_ID],
      ).toBe(experimentId);
    });
  });
  ...
  describe("Concurrent Experiments", () => {
    it("should not mix attributes between concurrent experiments", async () => {
      ...
      const exp1Name =
        child1?.attributes[AgentInsightOtelSpanAttributes.EXPERIMENT_NAME];
      const exp2Name =
        child2?.attributes[AgentInsightOtelSpanAttributes.EXPERIMENT_NAME];
  ...
  describe("Environment Attribute", () => {
    it("should set experiment environment on ALL spans including root", async () => {
      ...
      expect(
        rootSpan?.attributes[AgentInsightOtelSpanAttributes.ENVIRONMENT],
      ).toBe(AGENTINSIGHT_SDK_EXPERIMENT_ENVIRONMENT);
      expect(
        level1?.attributes[AgentInsightOtelSpanAttributes.ENVIRONMENT],
      ).toBe(AGENTINSIGHT_SDK_EXPERIMENT_ENVIRONMENT);
      expect(
        level2?.attributes[AgentInsightOtelSpanAttributes.ENVIRONMENT],
      ).toBe(AGENTINSIGHT_SDK_EXPERIMENT_ENVIRONMENT);
      expect(
        level3?.attributes[AgentInsightOtelSpanAttributes.ENVIRONMENT],
      ).toBe(AGENTINSIGHT_SDK_EXPERIMENT_ENVIRONMENT);
    });
    ...
    it("should set experiment environment value to 'sdk-experiment'", async () => {
      ...
      expect(
        childSpan?.attributes[AgentInsightOtelSpanAttributes.ENVIRONMENT],
      ).toBe("sdk-experiment");
    });
  });
