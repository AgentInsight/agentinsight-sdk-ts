...
import { ScoreManager } from "@agentinsight-sdk/client";
import {
...
function createScoreManager(mockAPIClient: MockAPIClient): ScoreManager {
  return new ScoreManager({
    apiClient: mockAPIClient as unknown as AgentInsightAPIClient,
  });
}
...
describe("ScoreManager Integration Tests", () => {
  ...
  describe("Basic Functionality", () => {
    ...
    it("should set environment from environment variable", async () => {
      const scoreManager = createScoreManager(mockAPIClient);
      process.env.AGENTINSIGHT_TRACING_ENVIRONMENT = "test-env";
      ...
      delete process.env.AGENTINSIGHT_TRACING_ENVIRONMENT;
    });
  });
  ...
  describe("Flush Logic", () => {
    it("should flush when reaching flushAtCount", async () => {
      process.env.AGENTINSIGHT_FLUSH_AT = "3";
      const scoreManager = createScoreManager(mockAPIClient);
      ...
      delete process.env.AGENTINSIGHT_FLUSH_AT;
    });

    it("should flush on timer when below flushAtCount", async () => {
      process.env.AGENTINSIGHT_FLUSH_INTERVAL = "0.1"; // 100ms
      const scoreManager = createScoreManager(mockAPIClient);
  ...
  describe("Timer Management", () => {
    it("should clear timer when manual flush is called", async () => {
      process.env.AGENTINSIGHT_FLUSH_INTERVAL = "1"; // 1 second
      const scoreManager = createScoreManager(mockAPIClient);
    ...
    it("should not create multiple timers for multiple scores", async () => {
      process.env.AGENTINSIGHT_FLUSH_INTERVAL = "0.2"; // 200ms
      const scoreManager = createScoreManager(mockAPIClient);
  ...
  describe("Configuration", () => {
    ...
    it("should use environment configuration values", () => {
      process.env.AGENTINSIGHT_FLUSH_AT = "25";
      process.env.AGENTINSIGHT_FLUSH_INTERVAL = "5";
  ...
  describe("Edge Cases and Additional Coverage", () => {
    it("should handle environment override in create method", async () => {
      const scoreManager = createScoreManager(mockAPIClient);
      process.env.AGENTINSIGHT_TRACING_ENVIRONMENT = "default-env";
    ...
    it("should handle invalid environment variable numbers gracefully", async () => {
      process.env.AGENTINSIGHT_FLUSH_AT = "invalid-number";
      process.env.AGENTINSIGHT_FLUSH_INTERVAL = "also-invalid";
