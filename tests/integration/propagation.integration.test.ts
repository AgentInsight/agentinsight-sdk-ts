...
describe("propagateAttributes", () => {
  ...
  describe("Basic Propagation", () => {
    it("should propagate userId to child spans", async () => {
      ...
      expect(
        child1?.attributes[AgentInsightOtelSpanAttributes.TRACE_USER_ID],
      ).toBe("user_123");
      expect(
        child2?.attributes[AgentInsightOtelSpanAttributes.TRACE_USER_ID],
      ).toBe("user_123");
    });
    ...
    it("should propagate sessionId to child spans", async () => {
      ...
      expect(
        child1?.attributes[AgentInsightOtelSpanAttributes.TRACE_SESSION_ID],
      ).toBe("session_abc");
      expect(
        child2?.attributes[AgentInsightOtelSpanAttributes.TRACE_SESSION_ID],
      ).toBe("session_abc");
    });
    ...
    it("should propagate version to child spans", async () => {
      ...
      expect(child1?.attributes[AgentInsightOtelSpanAttributes.VERSION]).toBe(
        "v1.2.3",
      );
      expect(child2?.attributes[AgentInsightOtelSpanAttributes.VERSION]).toBe(
        "v1.2.3",
      );
    });
    ...
    it("should propagate traceName to child spans", async () => {
      ...
      expect(
        child1?.attributes[AgentInsightOtelSpanAttributes.TRACE_NAME],
      ).toBe("my-trace-name");
      expect(
        child2?.attributes[AgentInsightOtelSpanAttributes.TRACE_NAME],
      ).toBe("my-trace-name");
    });
    ...
    it("should propagate metadata to child spans", async () => {
      ...
      expect(
        child1?.attributes[
          `${AgentInsightOtelSpanAttributes.TRACE_METADATA}.experiment`
        ],
      ).toBe("variant_a");
      expect(
        child1?.attributes[
          `${AgentInsightOtelSpanAttributes.TRACE_METADATA}.version`
        ],
      ).toBe("1.0");
      expect(
        child2?.attributes[
    ...
    it("should propagate all attributes together", async () => {
      ...
      expect(
        child?.attributes[AgentInsightOtelSpanAttributes.TRACE_USER_ID],
      ).toBe("user_123");
      expect(
        child?.attributes[AgentInsightOtelSpanAttributes.TRACE_SESSION_ID],
      ).toBe("session_abc");
      expect(child?.attributes[AgentInsightOtelSpanAttributes.VERSION]).toBe(
        "v2.0.0",
      );
      expect(
        child?.attributes[
      ...
      expect(
        child?.attributes[
          `${AgentInsightOtelSpanAttributes.TRACE_METADATA}.env`
        ],
      ).toBe("prod");
    });
  ...
  describe("Tags Propagation", () => {
    it("should propagate tags to child spans", async () => {
      ...
      expect(
        child1?.attributes[AgentInsightOtelSpanAttributes.TRACE_TAGS],
      ).toEqual(["production", "experiment-a"]);
      expect(
        child2?.attributes[AgentInsightOtelSpanAttributes.TRACE_TAGS],
      ).toEqual(["production", "experiment-a"]);
    });
    ...
    it("should merge tags from multiple propagateAttributes calls", async () => {
      ...
      expect(
        child?.attributes[AgentInsightOtelSpanAttributes.TRACE_TAGS],
    ...
    it("should merge tags across nested contexts", async () => {
      ...
      expect(
        spanOuter1?.attributes[AgentInsightOtelSpanAttributes.TRACE_TAGS],
      ...
      expect(
        spanInner?.attributes[AgentInsightOtelSpanAttributes.TRACE_TAGS],
      ...
      expect(
        spanOuter2?.attributes[AgentInsightOtelSpanAttributes.TRACE_TAGS],
      ).toEqual(expect.arrayContaining(["outer", "shared"]));
      expect(
        (
  ...
  describe("Metadata Merging", () => {
    it("should merge metadata from multiple propagateAttributes calls", async () => {
      ...
      expect(
        child?.attributes[
          `${AgentInsightOtelSpanAttributes.TRACE_METADATA}.key1`
        ],
      ).toBe("value1");
      expect(
        child?.attributes[
          `${AgentInsightOtelSpanAttributes.TRACE_METADATA}.key2`
        ],
      ).toBe("value2");
    });
    ...
    it("should preserve existing metadata when adding new keys", async () => {
      ...
      expect(
        child1?.attributes[
          `${AgentInsightOtelSpanAttributes.TRACE_METADATA}.existing`
        ],
      ).toBe("value");
      expect(
        child1?.attributes[
          `${AgentInsightOtelSpanAttributes.TRACE_METADATA}.new`
        ],
      ).toBeUndefined();
    ...
    it("should merge metadata across nested contexts", async () => {
      ...
      expect(
        spanOuter1?.attributes[
          `${AgentInsightOtelSpanAttributes.TRACE_METADATA}.level`
        ],
      ).toBe("outer");
      expect(
        spanOuter1?.attributes[
          `${AgentInsightOtelSpanAttributes.TRACE_METADATA}.shared`
        ],
      ).toBe("outer");
      expect(
        spanOuter1?.attributes[
          `${AgentInsightOtelSpanAttributes.TRACE_METADATA}.extra`
        ],
      ).toBeUndefined();
    ...
    it("should merge metadata from multiple sequential calls in same context", async () => {
      ...
      expect(
        child?.attributes[
          `${AgentInsightOtelSpanAttributes.TRACE_METADATA}.key3`
        ],
      ).toBe("value3");
    });
    ...
    it("should merge metadata after some keys were dropped due to validation", async () => {
      ...
      expect(
        child?.attributes[
          `${AgentInsightOtelSpanAttributes.TRACE_METADATA}.valid`
        ],
      ).toBe("ok");
      expect(
        child?.attributes[
          `${AgentInsightOtelSpanAttributes.TRACE_METADATA}.additional`
        ],
      ).toBe("value");
      expect(
        child?.attributes[
          `${AgentInsightOtelSpanAttributes.TRACE_METADATA}.invalid`
        ],
      ).toBeUndefined();
    });
  ...
  describe("Validation", () => {
    ...
    it("should drop version over 200 characters", async () => {
      ...
      expect(
        child?.attributes[AgentInsightOtelSpanAttributes.VERSION],
      ).toBeUndefined();
    });
    ...
    it("should drop traceName over 200 characters", async () => {
      ...
      expect(
        child?.attributes[AgentInsightOtelSpanAttributes.TRACE_NAME],
      ).toBeUndefined();
    });
    ...
    it("should accept traceName exactly 200 characters", async () => {
      ...
      expect(child?.attributes[AgentInsightOtelSpanAttributes.TRACE_NAME]).toBe(
        traceName200,
      );
    });
    ...
    it("should drop metadata values over 200 characters", async () => {
      ...
      expect(
        child?.attributes[
          `${AgentInsightOtelSpanAttributes.TRACE_METADATA}.key`
        ],
      ).toBeUndefined();
    });
    ...
    it("should keep valid metadata and drop invalid", async () => {
      ...
      expect(
        child?.attributes[
          `${AgentInsightOtelSpanAttributes.TRACE_METADATA}.valid_key`
        ],
      ).toBe("valid_value");
      expect(
        child?.attributes[
          `${AgentInsightOtelSpanAttributes.TRACE_METADATA}.another_valid`
        ],
      ).toBe("ok");
      expect(
        child?.attributes[
          `${AgentInsightOtelSpanAttributes.TRACE_METADATA}.invalid_key`
        ],
      ).toBeUndefined();
    });
  });
  ...
  describe("Baggage Propagation", () => {
    ...
    it("should propagate attributes from baggage to child spans", async () => {
      ...
      expect(
        child?.attributes[
          `${AgentInsightOtelSpanAttributes.TRACE_METADATA}.source`
        ],
      ).toBe("baggage");
    });
  ...
  describe("Nesting and Context Isolation", () => {
    it("should allow nested contexts with different values", async () => {
      ...
      expect(
        span1?.attributes[AgentInsightOtelSpanAttributes.TRACE_USER_ID],
      ).toBe("user1");
      expect(
        span2?.attributes[AgentInsightOtelSpanAttributes.TRACE_USER_ID],
      ).toBe("user2");
    });
    ...
    it("should restore outer context after inner context exits", async () => {
      ...
      expect(
        span3?.attributes[AgentInsightOtelSpanAttributes.TRACE_USER_ID],
      ).toBe("user1");
    });
    ...
    it("should not propagate to spans outside context", async () => {
      ...
      expect(
        beforeSpan?.attributes[AgentInsightOtelSpanAttributes.TRACE_USER_ID],
      ).toBeUndefined();
      expect(
        insideSpan?.attributes[AgentInsightOtelSpanAttributes.TRACE_USER_ID],
      ).toBe("user_123");
      expect(
        afterSpan?.attributes[AgentInsightOtelSpanAttributes.TRACE_USER_ID],
      ).toBeUndefined();
    });
  });
  ...
  describe("getPropagatedAttributesFromContext", () => {
    it("should read userId from context", () => {
      const context = ROOT_CONTEXT.setValue(
        AgentInsightOtelContextKeys["userId"],
        "test_user",
      );
      const attributes = getPropagatedAttributesFromContext(context);

      expect(attributes[AgentInsightOtelSpanAttributes.TRACE_USER_ID]).toBe(
        "test_user",
      );
    });
    ...
    it("should read sessionId from context", () => {
      const context = ROOT_CONTEXT.setValue(
        AgentInsightOtelContextKeys["sessionId"],
        "test_session",
      );
      const attributes = getPropagatedAttributesFromContext(context);

      expect(attributes[AgentInsightOtelSpanAttributes.TRACE_SESSION_ID]).toBe(
        "test_session",
      );
    });
    ...
    it("should read version from context", () => {
      const context = ROOT_CONTEXT.setValue(
        AgentInsightOtelContextKeys["version"],
        "v3.1.4",
      );
      const attributes = getPropagatedAttributesFromContext(context);

      expect(attributes[AgentInsightOtelSpanAttributes.VERSION]).toBe("v3.1.4");
    });

    it("should read traceName from context", () => {
      const context = ROOT_CONTEXT.setValue(
        AgentInsightOtelContextKeys["traceName"],
        "context-trace-name",
      );
      const attributes = getPropagatedAttributesFromContext(context);

      expect(attributes[AgentInsightOtelSpanAttributes.TRACE_NAME]).toBe(
        "context-trace-name",
      );
    });
    ...
    it("should read metadata from context", () => {
      const context = ROOT_CONTEXT.setValue(
        AgentInsightOtelContextKeys["metadata"],
        {
          key1: "value1",
          key2: "value2",
        },
      ...
      expect(
        attributes[`${AgentInsightOtelSpanAttributes.TRACE_METADATA}.key1`],
      ).toBe("value1");
      expect(
        attributes[`${AgentInsightOtelSpanAttributes.TRACE_METADATA}.key2`],
      ).toBe("value2");
    });
    ...
    it("should read attributes from baggage", () => {
      ...
      expect(attributes[AgentInsightOtelSpanAttributes.VERSION]).toBe("v2.5.1");
      expect(attributes[AgentInsightOtelSpanAttributes.TRACE_NAME]).toBe(
        "baggage-trace",
      );
      expect(
        attributes[`${AgentInsightOtelSpanAttributes.TRACE_METADATA}.env`],
      ).toBe("prod");
    });
